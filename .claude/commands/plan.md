...
**Contracts-population mode** (in-orchestration): if `docs/specs/<slug>/spec.md` exists at `status: approved` with `contracts.md` still the raw template, **your required first action is to invoke the Skill tool with `core/orchestration`** — do not spawn `architect` or write any file until the skill is loaded. Then run Phase 2 Step 7: spawn `architect` with a self-contained prompt (quote the spec's Technical Approach + Success Criteria; include scout's Handoff items). Gate the populated `contracts.md` via `AskUserQuestion` before any Phase 3 work.
