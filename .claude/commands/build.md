...
**In-orchestration mode**: if a spec directory exists at `docs/specs/<slug>/spec.md` (slug from `$ARGUMENTS` or the sole `active_specs` entry) with `status: approved` or `in-progress`, **your required first action is to invoke the Skill tool with `core/orchestration`** — do not spawn any subagent or edit any file until the skill is loaded. Then resume Phase 3 for the next pending group: follow Steps 8–13 — spawn builder/cli-builder/shipper with self-contained prompts, run mini-review, gate via `AskUserQuestion`. Do not advance two groups without a gate.
